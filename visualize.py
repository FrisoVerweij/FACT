...
def visualize(config):
    ...
    Xhats, Yhats = generate_sampels(z_dim, nsamples, latentsweep_vals, x_val, sample_ind, device, encoder, decoder, classifier)

    create_figures(z_dim, nsamples, latentsweep_vals, Xhats, Yhats, cols, border_size)

    print('Columns - latent values in sweep: ' + str(latentsweep_vals))
    print('Rows - sample indices in vaX: ' + str(sample_ind))


def generate_sampels(z_dim, nsamples, latentsweep_vals, x_val, sample_ind, device, encoder, decoder, classifier):
...
def create_figures(z_dim, nsamples, latentsweep_vals, Xhats, Yhats, cols, border_size):
    for latent_dim in range(z_dim):
        ...
        for isamp in range(nsamples):
            for (ilatentsweep, latentsweep_val) in enumerate(latentsweep_vals):
                ...
                img_bordered = np.tile(np.expand_dims(np.array(cols[yhat]), (0, 1)),
                                       (config['image_size'] + 2 * border_size, config['image_size'] + 2 * border_size,
                                        1))
