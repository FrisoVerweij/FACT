...
class Encoder(nn.Module):
    ...
    def __init__(self, n_beta, n_alpha, x_dim,
                 filt_per_layer=64):  # x_dim : total number of pixels
        super(Encoder, self).__init__()
        self.model = nn.Sequential(
            nn.Conv2d(int(n_alpha), filt_per_layer, 4, stride=2, padding=1),
            nn.ReLU(),
        ...
        self.fc_mu = nn.Linear(int(filt_per_layer * x_dim / 16), n_beta)
        self.fc_logvar = nn.Linear(int(filt_per_layer * x_dim / 16), n_beta)
...
class Decoder(nn.Module):
    ...
    def __init__(self, n_beta, n_alpha, x_dim,  # x_dim : total number of pixels
                 filt_per_layer=64):
        ...
        self.fc = nn.Sequential(
            nn.Linear(n_beta, int(filt_per_layer * self.x_dim / 16)),
            nn.ReLU()
        )
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(filt_per_layer, int(n_alpha), 4, stride=2, padding=1),
            nn.Sigmoid()
        )
